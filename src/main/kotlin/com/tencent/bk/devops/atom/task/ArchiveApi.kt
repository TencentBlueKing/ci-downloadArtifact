...
class ArchiveApi : BaseApi() {
    ...
    fun downloadPipelineFile(userId: String, projectId: String, pipelineId: String, buildId: String, uri: String, destPath: File, size: Long) {
        logger.info("downloadPipelineFile: userId: $userId, projectId: $projectId, pipelineId: $pipelineId, buildId: $buildId, uri: $uri, destPath: $destPath")
    ...
    fun downloadCustomFile(userId: String, projectId: String, uri: String, destPath: File, size: Long) {
        logger.info("downloadPipelineFile: userId: $userId, projectId: $projectId, destPath: $destPath, uri: $uri, size: $size")
        val url = "/bkrepo/api/build/generic/$projectId/custom/$uri"
        var header = HashMap<String, String>()
        header.set("X-BKREPO-UID", userId)
        val request = buildGet(url, header)
        downloadFile(request, destPath)
    ...
    fun downloadFile(
        ...
        try {
            ...
            for (patters in paths) {
                ...
                var jfrogFiles = matchFile(user, targetProjectId, "pipeline", path, targetPipelineId, buildId)
